...
//! One of these objects should be added to the scene corresponding to each Haptic Device you intend the scene to connect to. 
...
public class HapticPlugin : MonoBehaviour  {
	...
	void FixedUpdate() 
	{
        ...
        //La force ajoutée manuellement provoque deux saccades initiales deja, et après augmente linéairement. Ca démarre assez doucement aussi. (x --> de gauche à droite)
        /*
        double[] identité = { 1.0, 0.0, 0.0 };
        setForce("Default PHANToM", identité, identité);
        */
        Debug.Log("(" + hapticManipulator.transform.position.x + ", " + hapticManipulator.transform.position.y + ", " + hapticManipulator.transform.position.z + ")");
        checkErrors();
	}
	...
	private void updateManipulator()
	{
		...
		{ // No physics, just move it.
            ...
            //Debug.Log("(" + hapticManipulator.transform.position.x + ", " + hapticManipulator.transform.position.y + ", " + hapticManipulator.transform.position.z + ")");
            //D'où viennent les oscillations du stylet graphique (le grabber)?
            return;
		}
		previousManipulator = hapticManipulator;
		...
		setSpringStiffness(configName, PhysicsForceStrength, 0.0);
...

		{
			...
			force *= body.mass;
			//force *= 1000;
			force *= 150; // Magic number So that manipulator with mass 1.0 feels right. Du coup autant regrouper les deux coeff, il serait intéressant de savoir d'ou vient le 1000 ?
	...
	public void safeUpdateManipulator()
	{
		...
		if (body == null)
			return;
		updateDevice();
		if (stylusPositionWorld.x == stylusPositionWorld.x)
		{
			body.position = stylusPositionWorld;
			body.rotation = stylusRotationWorld;
	...
	public static double[] MatrixToDoubleArray(Matrix4x4 M)
	{
	...
	void OnDrawGizmos()
	{
		if (hHD >= 0)
		{
			...
			Gizmos.DrawWireCube(new Vector3 (0, 0, 20), new Vector3 (2, 2, 40));
    ...
    /*public void UpdateStylusDecalage()
    {
        Vector3 Decalage = stylusPositionRaw - stylusPositionWorld;
    }
    */
} //class HapticPlugin
